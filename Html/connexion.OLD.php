...
class PdoBdd {   		
    ...
    */
    ...
    {
        $pdo = PdoBdd::getPdoBdd();
        $stmt = $pdo->prepare("DELETE FROM task WHERE userid = :user AND taskdate = :date
            AND taskam = :am");
        $stmt->bindParam(':user', $user);
    ...
    {
        $pdo = PdoBdd::getPdoBdd();
        $stmt = $pdo->prepare("SELECT * FROM task WHERE userid = :user AND taskdate = 
        ...
        {
            $pdo = PdoBdd::getPdoBdd();
            $stmt = $pdo->prepare('DELETE FROM projectuser WHERE userid = :user AND projectid = :project');
            $stmt->bindParam(':user', $user);
            $stmt->bindParam(':project', $idProject);
            $stmt->execute();
            PdoBdd::updateUserVersion($user);
        ...
        {
            $pdo = PdoBdd::getPdoBdd();
            $stmt = $pdo->prepare('DELETE FROM activityuser WHERE userid = :user 
        ...
        {
            $pdo = PdoBdd::getPdoBdd();
             $stmt = $pdo ->prepare("SELECT distinct M.userid
             from manager as M
             natural join project as P
             where M.userid = :user;");
             $stmt->bindParam(':user',$user,PDO::PARAM_INT);
        ...
        {
           $pdo = PdoBdd::getPdoBdd();
            $stmt = $pdo ->prepare("SELECT TL.userid as userid, T.label as label
        ...
        */
        function updateUserVersion($user)
        {
            ...
            $centralversion++;
            $pdo = PdoBdd::getPdoBdd();
            $stmt = $pdo->prepare("UPDATE userversion SET centralversion = :centralversion WHERE userid=:user");
            $stmt->bindParam(':user', $user);
